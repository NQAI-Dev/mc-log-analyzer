...
RULES = [
    {
        ...
        "pattern": re.compile(r"You need to agree to the EULA in order to run the server", re.IGNORECASE),
    ...
    },
    {
        ...
        "pattern": re.compile(r"(FAILED TO BIND TO PORT|Address already in use: bind|BindException: Address already in use)", re.IGNORECASE),
    ...
    },
    {
        ...
        "pattern": re.compile(r"has been compiled by a more recent version of the Java Runtime \(class file version (\d+\.\d+)\), this version of the Java Runtime only recognizes class file versions up to (\d+\.\d+)", re.IGNORECASE),
    ...
    {
        "id": "oom_heap",
        "pattern": re.compile(r"(java\.lang\.OutOfMemoryError:\s*Java heap space|Out of memory: Kill process)", re.IGNORECASE),
    ...
    {
        "id": "oom_metaspace",
        "pattern": re.compile(r"java\.lang\.OutOfMemoryError:\s*Metaspace", re.IGNORECASE),
    ...
    },
    {
        ...
        "pattern": re.compile(r"(?:Could not load '[^']*' in folder 'plugins'|UnknownDependencyException|Plugin '[^']+' requires|depends on: )", re.IGNORECASE),
    ...
    {
        "id": "corrupted_chunk",
        "pattern": re.compile(r"(Chunk file at .*? is in the wrong location|Corrupt chunk detected|RegionFileException|Corrupted chunk data)", re.IGNORECASE),
    ...
    {
        "id": "sqlite_db_locked",
        "pattern": re.compile(r"(database is locked|sqlite3\.OperationalError:\s*database is locked)", re.IGNORECASE),
